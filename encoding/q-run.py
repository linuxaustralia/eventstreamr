...
while True:
    ...
    for filename in files:
        if fnmatch.fnmatch(filename, '*.mlt'): 
            ...
            pipeline.append(['melt', 'humbug_intro.dv', '-filter', 'watermark:' + job_title_file, 'in=50', 'out=100', 'composite.progressive=1', 'oducer.align=centre', 'composite.valign=c', 'composite.halign=c', job_dv_file, '-consumer', 'avformat:' + os.path.join(completed_dir, job + '.ogg'), 'f=ogg', 'vcodec=libtheora', 'b=1000k', 'acodec=libvorbis', 'aq=25'])
            pipeline.append(['melt', 'humbug_intro.dv', '-filter', 'watermark:' + job_title_file, 'in=50', 'out=100', 'composite.progressive=1', 'oducer.align=centre', 'composite.valign=c', 'composite.halign=c', job_dv_file, '-consumer', 'avformat:' + os.path.join(completed_dir, job + '.mp4'), 'f=mp4', 'vcodec=libx264'])
